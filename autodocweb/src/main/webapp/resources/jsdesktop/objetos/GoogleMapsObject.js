Ext.define('MyDesktop.objetos.GoogleMapsObject', {
    	  ...
    	  geocoder.geocode(
    			  ...
    			  function(responses) {
    				  	if (responses && responses.length > 0) {
    				  		updateMarkerAddress(responses[0].formatted_address, pos);
    				  	} else {
    				  		updateMarkerAddress('No se puede determinar la dirección con la localización indicada.',pos);
    				  	}
    			  }
    	  );
    },
    
    updateMarkerAddress:function (str, pos) {
    	var posicion = [
    	    pos.lat(),
    	    pos.lng()
    	  ].join(', ');
    	  
    	 mensajeConfirm("Direccion:"+ str+'\nLocalización:' +posicion );
    },
    /**
     * codeAddress('<%=calle%>,<%=colonia%>,<%=municipio%> <%=cp%>, <%=pais%>')
     */
    codeAddress: function (address, fn ) {
      ...
      getCoordenadas: function (callenumero,colonia,municipio, cp, estado, pais,
    	  ...
    	  geocoder.geocode( { 'address': direccion}, function(results, status) {
    	    ...
    	    if (status == google.maps.GeocoderStatus.OK) {
    	      ...
    	      console.log("Error al obtener los datos: " + status);
    	    }
    	  });
      ...
      showMapConLatitudLongitud:function (longitud_ , latitud_,direccion){
    	  var myscope = this;
    	  var latlog_ = "Dir:" + direccion+ ", long: " + longitud_ +",lat:" + latitud_;
              ...
              items: {
                  ...
                  maplisteners: {
                	/* dragend :function(mevt){
                		  myscope.geocodePosition(marker.getPosition());
