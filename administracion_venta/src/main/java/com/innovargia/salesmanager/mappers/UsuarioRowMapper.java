...
public class UsuarioRowMapper implements RowMapper<UsuarioDTO>{
	...
	public UsuarioDTO mapRow(ResultSet rs, int rowNum) throws SQLException {
		...
		obj.setPlaza(ResultSetUtils.getRSString("plaza", rs)); 
		obj.setPuesto(ResultSetUtils.getRSString("puesto", rs)); 
		obj.setNivelPuesto(ResultSetUtils.getRSInt("nivelPuesto", rs)); 
		
		obj.setDescripcionpuesto(UtilsStringFechas.convierteCadenaTrimNULL(ResultSetUtils.getRSString("descripcionpuesto",rs)));
