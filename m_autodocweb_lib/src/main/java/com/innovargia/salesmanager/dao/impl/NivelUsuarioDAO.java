...
public class NivelUsuarioDAO extends JdbcDaoSupport implements  INivelUsuarioDAO {
	...
	public List<NivelUsuarioDTO> getRegistrosNivelAgrupamiento(NivelUsuarioDTO catalogo)
		...
		String sql = "select caagrupadorjeraquiapuestos.*,canivelusuario.descripcion "
				...
				+ " idnivelusuario=idpuesto where idagrupamiento=?";
		
			
		lista = jdbcTemplate.query(sql,new Object[]{ catalogo.getIdagrupamiento()},new NivelUsuarioRowMapper());
		return lista;
	}
	...
	public NivelUsuarioDTO get(int nivel) throws Exception {
		...
		List<NivelUsuarioDTO> lista = new ArrayList<NivelUsuarioDTO>();
		String sql = "SELECT * FROM canivelusuario WHERE idnivelusuario=?";

		lista = jdbcTemplate.query(sql,new Object[]{nivel},new NivelUsuarioRowMapper());
		if (lista.size()>0){
			return lista.get(0);
		}
	...
	@Override
	public UsuarioDTO getPerfil(UsuarioDTO Usuario) {
		
		UsuarioDTO nuevo_usuario = new UsuarioDTO();
				
		String sql = "SELECT * FROM causuarios WHERE login ='" + Usuario.getLogin() +"'";
		
		try {
			nuevo_usuario = jdbcTemplate.queryForObject(sql, new UsuarioRowMapper());
	...
	@Override
	public String getRazonSocial(UsuarioDTO Usuario) {
		
		String razonSocial = "";
		
		String sql = "SELECT razonsocial FROM caclientesrfc WHERE numcliente ='" + Usuario.getNumCliente() +"'";
		
		try {
			
			razonSocial = jdbcTemplate.queryForObject(sql, String.class);
