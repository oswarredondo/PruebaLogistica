 ...
 * @version
 */
public class EntregaForm   implements Serializable   {
     ...
     */
    public EntregaForm(String idRuta,String origen) {
        setIdRuta(idRuta);
        setOrigen(origen);
    }
     ...
     */
    public EntregaForm(String idGuia,String fecha,int hora, int minutos){
        setIdGuia(idGuia);
        setFecha(fecha);
        setHora(hora);
        setMinutos(minutos);
     ...
     * @param hora hora en que se recibe
     */
    public EntregaForm(String idGuia,String quienRecibe,String fecha,int hora,int minutos) {
        setIdGuia(idGuia);
    ...
    /**
     * Da formato a la fecha para generar un registro de intercambio
     * para insertarlo por medio del WebService en el formato YYYYMMDDHHMM
     * @return una cadena con la fecha en formato yyyyMMddHHmm
     */
    public String getFechaConfirmacionRegInfo() {
        ...
        if (this.fecha!=null){
            ...
            String s_min = this.minutos+"";
            while (s_min.length()<2){
                s_min="0"+s_min;
            }
            String formato_sybase = fecha_elem[2]+fecha_elem[1] +fecha_elem[0] ;
            String s_hora=  this.hora+"";
             while (s_hora.length()<2){
                s_hora="0"+s_hora;
            }
            
            return  formato_sybase  + s_hora + s_min;
        }else{
            //Retorna la fecha del dia actual
            return Fechas.getFechaFormato("yyyyMMddHHmm");
        }
    }//fin de getFechaConfirmacionRegInfo
