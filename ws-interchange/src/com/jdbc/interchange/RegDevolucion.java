 ...
 * 
 */
public class RegDevolucion implements InterfaceUpdateInser {
	 ...
	 * 
	 */
	public RegDevolucion(String db) {
		this.db = db;
	}

	public RegDevolucion() {
	...
	public void procesa(String registro, String db) throws SQLException {
		...
		try {
			//13|NumGuia|Ruta|guiadevolucion|Fecha|OriReg|Plaza|Empleado
			int index=0;
			...
			confDao.insertDevolucion(seguimiento);

			// Se agrega la clave de excepcion para agregar un registro de
			// movimiento
			seguimiento.setClaveExce("");
			//seguimiento.setRuta("");
