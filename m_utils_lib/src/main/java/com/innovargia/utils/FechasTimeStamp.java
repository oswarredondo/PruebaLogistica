...
public class FechasTimeStamp extends Fechas {
	...
	public static java.sql.Timestamp convertFechaToTimeStamp(String fecha){
		...
		return setTimestamp(anio, mes, dia, hora, minutos,0);
	...
	public static java.sql.Timestamp convertFechaToTimeStamp(String fecha,
		...
		if (fecha==null) return null;
		if (fecha.trim().length()<=0) return null;
        
		//Quita todos los espacios de la fecha
		fecha=fecha.trim();
		int dia = 0;
		int mes = 0;
		int anio = 0;
		...
		String[] datos_fecha = fecha.trim().split(" "); 
		String[] partes_fecha =  datos_fecha[0].split(separador);
		String[] partes_hora = null;

		//Verifica si existe la hora indicada en la fecha la toma 
		//si no existe entonces toma la hora indicada como parametro
		if (datos_fecha.length<=1){
			//Asigna la hora indica por el usuario
			...
			//Asigna la hora que esta indicada en la fecha
			partes_hora = datos_fecha[1].split(":");
		}
		
		//Asigna datos de minutos y hora
		...
		//Si el formato es de este tipo saca la hora y fecha indicadas
		...
		return setTimestamp(anio, mes, dia, hora, minutos,segundos);
	...
	public static java.sql.Time formatoTime(String s_time ){
		...
		if (s_time.trim().length()<=0) return null;
	...
	public static java.sql.Date formatoDateDDMMAAAA(String s_date ){
		...
		if (s_date.trim().length()<=0) return null;
	 ...
	 * Se debe indicar el formato de la fecha "dd/MM/yyyy" o "dd-MM-yyyy"
	 * @param fecha (String) la fecha a la cual deseo convertir
	 * @return Date la fecha en formato Date
	 */
	 
	public static Date stringToDate(String fecha,String formato_str){
	        SimpleDateFormat formato = new SimpleDateFormat(formato_str);
