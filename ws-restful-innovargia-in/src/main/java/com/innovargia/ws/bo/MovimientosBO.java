...
import com.innovargia.ws.dto.SiguienteMovimiento;
			...
			if ((idguia != null) && (idguia.trim().length() > 0)) {
				if (idguia.trim().length() <= 30) {
					...
					try {
						...
						/*
						 * if (request.equals("200") && countGuias ==
						 * arr_guias.length) return request; if(countGuias ==
						 * arr_guias.length) return request;
						 */
						// countGuias++;
					} catch (Exception e) {
