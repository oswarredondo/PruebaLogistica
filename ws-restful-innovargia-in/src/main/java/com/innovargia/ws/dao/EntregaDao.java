...
import com.innovargia.ws.rowmap.GuiaRowMap;
import com.innovargia.ws.rowmap.RutaRowMap;
		...
		try {
			...
			if(salida.equals("5")){
				query = "INSERT INTO confirma (idguia,conombre,cofecha," + "fechainsercion,latitud,longitud, "
						//+ "ruta,plsiglasplaza,ceorigenregistro) " + "VALUES (:idguia,:nomRecibe,'" + obj.getFecha() + "',"
				...
				query = "INSERT INTO confirma (idguia, conombre, cofecha, fechainsercion, latitud, longitud, ruta, plsiglasplaza, ceorigenregistro) " +
						//"(SELECT iddocumento AS idguia, :nomRecibe AS conombre, '" + obj.getFecha() + "' AS cofecha, current_timestamp AS fechainsercion, :latitud AS latitud, :longitud AS longitud, " +
						...
						":ruta AS ruta, :plaza AS plsiglasplaza, :oriReg AS ceorigenregistro FROM cadocumentos WHERE iddocumentomadre IN ('" + obj.getIdguia() + "') AND iddocumento NOT IN ( " +
						"SELECT idguia FROM confirma WHERE idguia IN (SELECT iddocumento FROM cadocumentos WHERE iddocumentomadre IN ('" + obj.getIdguia() + "'))))";
				this.namedParameterJdbcTemplate.update(query, parameterSource);
			}
			/*
			 * 1 = Tiene entrega
			...
			switch(tiregistro){
				case "24":
					...
					if(count >= 1){
						...
						return "5"; //Si la guia no tiene entrega proceder al registro
					}
				default:
					return "2";
			}
